...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  if (request.action === "updateBlocking") {
    console.log("[Focux] Received updateBlocking message");
...
chrome.storage.onChanged.addListener((changes, areaName) => {
  if (areaName === "local" && changes["focux-websites-2m31"]) {
    console.log("[Focux] Storage changed, updating blocking status");
    checkAndUpdateBlocking();
  }
});
...
document.addEventListener("visibilitychange", () => {
  if (document.visibilityState === "visible") {
    console.log("[Focux] Tab became visible, checking blocking status");
    checkAndUpdateBlocking();
  }
});